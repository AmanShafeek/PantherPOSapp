...
ipcMain.handle('db-query', async (event, sql, params) => {
    try {
        if (!dbModule)
            throw new Error('DB Module not loaded');
...
ipcMain.handle('scale-read-weight', async () => {
    try {
        ...
        const settings = await dbModule.query('SELECT * FROM settings WHERE key IN (?, ?, ?, ?)', ['scale_enabled', 'scale_port', 'scale_baud_rate', 'scale_protocol']);
        const scaleSettings = {};
...
ipcMain.handle('open-window', async (event, windowPath) => {
...
ipcMain.handle('db-backup-create', () => {
    if (!dbModule)
        return { success: false, error: 'DB not loaded' };
...
ipcMain.handle('db-backup-restore', (event, filename) => {
    if (!dbModule)
        return { success: false, error: 'DB not loaded' };
...
ipcMain.handle('audit-log-add', (event, { userId, userName, action, details }) => {
    if (!dbModule)
        return { success: false };
    return dbModule.addAuditLog(userId, userName, action, details);
});
ipcMain.handle('audit-log-list', (event, limit) => {
    if (!dbModule)
        return [];
    return dbModule.getAuditLogs(limit);
});
// Notifications
ipcMain.handle('notification-add', (event, payload) => {
    return dbModule.addNotification(payload.type, payload.title, payload.message);
});
ipcMain.handle('notification-list', (event, unreadOnly) => {
    return dbModule.getNotifications(unreadOnly);
});
ipcMain.handle('notification-read', (event, id) => {
